        ...
        for path in data_paths:
            try:
                if os.path.exists(path):
                    ...
                    st.success(f"✅ 成功从 {path} 加载数据")
                    break
            except Exception as e:
                continue
        
        if df is None:
            st.error("❌ 未找到数据文件，请确保 '两版合并后的年报数据_完整版.xlsx' 在正确的位置")
            return None
...
if df is not None:
    ...
    with col_left:
        ...
        fig_pie.update_layout(height=400, width=None)
    ...
    with col_right:
        ...
        fig_bar.update_layout(height=400, width=None)
    ...
    fig_company_bar.update_layout(height=500, width=None)
...
st.markdown(
    ...
    <div style='text-align: center; color: gray;'>
        <p>📊 企业数字化转型指数查询系统 | Powered by Streamlit</p>
        <p>数据来源: 年报数据 | 最近更新: 2024年</p>
    </div>
    """, 
